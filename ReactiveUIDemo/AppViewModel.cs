...
namespace ReactiveUIDemo
{
    public sealed class AppViewModel : ReactiveObject
    {
        ...
        public AppViewModel()
        {
            ...
            _searchResults.ThrownExceptions.Subscribe(error => Console.WriteLine(error));
        ...
        private async Task<IEnumerable<NugetDetailsViewModel>> SearchNuGetPackages(string? term, CancellationToken cancellationToken)
        {
            ...
            var metadata = await resource.SearchAsync(term, filter, 0, 10, logger, cancellationToken).ConfigureAwait(false);
